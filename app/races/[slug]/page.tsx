...
export default async function RaceDetailPage({
  ...
  const getDifficultyColor = (difficulty: string) => {
    switch (difficulty) {
      ...
      case "moderate":
        return "bg-[#CC7722]/30 text-[#CC7722] border-[#CC7722]";
      case "hard":
  ...
  return (
    <>
      {/* View Tracker - renders nothing, just tracks */}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-16">
              ...
              <div className="hidden md:flex space-x-8">
                ...
                <Link href="/races" className="text-[#CC7722] font-semibold">
                  Discover Races
                </Link>
                <Link
                  href="/about"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="h-96 bg-gradient-to-br from-[#4A7BA7] via-[#5A7247] to-[#CC7722] rounded-3xl relative overflow-hidden shadow-2xl">
            ...
            <div className="absolute top-6 right-6 flex gap-3">
              <span
                className={`px-5 py-2 rounded-full text-sm font-bold border-2 backdrop-blur-sm ${getDifficultyColor(
                  race.difficulty
                )}`}
              >
                {race.difficulty.charAt(0).toUpperCase() +
                  race.difficulty.slice(1)}
              </span>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              {/* Title & Basic Info */}
              <div className="bg-white rounded-2xl p-8 border border-[#C2B280]/20 shadow-sm relative">
                <h1 className="text-4xl md:text-5xl font-bold text-[#2D2D2D] mb-6 leading-tight pr-32">
                  {race.name}
                </h1>
                <div className="absolute top-6 right-6">
                  <ViewCount count={race.view_count} className="text-base" />
                </div>
                          ...
                          {dist.entry_fee && (
                            <div className="flex items-center gap-2 text-[#5A5A5A]">
                              ...
                              <span className="font-semibold">
                                R{dist.entry_fee.toLocaleString()}
                              </span>
                            </div>
                          )}
                          {dist.slots && (
                            <div className="flex items-center gap-2 text-[#5A5A5A]">
                              ...
                              <span>
                                {dist.slots.toLocaleString()} slots available
                              </span>
                            </div>
                          )}
                        </div>
              ...
              <div className="bg-white rounded-2xl p-8 border border-[#C2B280]/20 shadow-sm">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="bg-[#FAF7F0] p-5 rounded-xl border border-[#C2B280]/20">
                    ...
                    <p className="text-lg text-[#2D2D2D] capitalize font-bold">
                      {race.terrain}
                    </p>
                  </div>
                  ...
                  {race.elevation_gain && (
                    <div className="bg-[#FAF7F0] p-5 rounded-xl border border-[#C2B280]/20">
                      ...
                      <p className="text-lg text-[#2D2D2D] font-bold">
                        {race.elevation_gain.toLocaleString()}m
                      </p>
                    </div>
                  )}
                  ...
                  {race.entry_opens_date && (
                    <div className="bg-[#FAF7F0] p-5 rounded-xl border border-[#C2B280]/20">
                      ...
                      <p className="text-sm text-[#2D2D2D] font-bold">
                        {formatDate(race.entry_opens_date)}
                      </p>
                    </div>
                  )}
                  ...
                  {race.entry_closes_date && (
                    <div className="bg-[#FAF7F0] p-5 rounded-xl border border-[#C2B280]/20">
                      ...
                      <p className="text-sm text-[#2D2D2D] font-bold">
                        {formatDate(race.entry_closes_date)}
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 space-y-6">
                ...
                <div className="bg-gradient-to-br from-[#CC7722] to-[#E67E22] rounded-2xl p-8 text-white shadow-xl">
                  <h3 className="text-2xl font-bold mb-3">Ready to Race?</h3>
                ...
                <div className="bg-white rounded-2xl p-6 border border-[#C2B280]/20 shadow-sm">
                  <h3 className="font-bold text-[#2D2D2D] mb-4 text-lg">
                    Quick Facts
                  </h3>
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center py-2 border-b border-[#C2B280]/20">
                      ...
                      <span
                        className={`font-bold text-sm px-3 py-1 rounded-full ${getDifficultyColor(
                          race.difficulty
                        )}`}
                      >
                        {race.difficulty.charAt(0).toUpperCase() +
                    ...
                    {race.distances && race.distances.length > 0 && (
                      <div className="flex justify-between items-center py-2">
                        ...
                        <span className="font-bold text-[#2D2D2D] text-sm">
                          {race.distances.length} option
                          {race.distances.length > 1 ? "s" : ""}
                        </span>
                      </div>
                    )}
                  </div>
